...
game.tryToShoot = function(){
    ...
    for(var i=0; i<activeBases.length; i++){
...
var level = {
    ...
    init: function(){
        ...
        for(var i=0;i<3;i++){
            var b = new Base(i);//settings.CW/105*(i*45+7.5));
            game.bases.push(b);
        }
    },
    update: function(){
...
function Base(num){
    ...
    this.ammo = 10;
    this.idx = num;
    this.x = gDATA.bases[num].x;
    this.y = gDATA.bases[num].y;
    this.rockets = [];
}
...
function Rocket(origin, target){
    ...
    if(origin.y < settings.CH*0.82){
        this.speed = settings.speed-Math.random()*2;
    } else {
...
function Explosion(where){
    ...
    this.active = true;
    this.counter = 0;
    this.fullRadius = (settings.CW+settings.CH)/50;
    this.radius = 0;
}
